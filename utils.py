...
def associate_dual_victory_to_person(midpoint_x: int, midpoint_y: int, 
                                    hand1_landmarks: List[Tuple[int, int]], 
                                    hand2_landmarks: List[Tuple[int, int]],
                                    boxes_xyxy: np.ndarray, 
                                    frame_shape: Tuple[int, int]) -> Optional[int]:
    """
    Associate dual victory gesture combination to the nearest person.
    Uses midpoint between hands as primary association point.
    ...
    for i, box in enumerate(boxes_xyxy):
        expanded_box = expand_box(box, expansion_factor=0.5)  # Larger expansion for two hands
        if point_in_box(midpoint_x, midpoint_y, expanded_box):
            print(f"[ASSOC] Dual Victory midpoint ({midpoint_x},{midpoint_y}) -> Person {i} (expanded box)")
            return i
    ...
    for i, box in enumerate(boxes_xyxy):
        ...
        if hand1_near and hand2_near:
            print(f"[ASSOC] Both victory hands near Person {i}")
            return i
    ...
    if best_idx is not None:
        print(f"[ASSOC] Dual Victory midpoint ({midpoint_x},{midpoint_y}) -> Person {best_idx} (nearest, dist={best_distance:.1f})")
        return best_idx
    
    print(f"[ASSOC] Dual Victory midpoint ({midpoint_x},{midpoint_y}) -> No person associated")
    return None
